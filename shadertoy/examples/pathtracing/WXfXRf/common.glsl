...
vec4 clampg(vec4 p, float a, float b) {
  return vec4(
    clampp(p.w, a, b) * p.xyz,
    clampe(p.w, a, b)); }

mat4 clampg(mat4 p, vec3 a, vec3 b) {
  return mat4(
    clampg(p[0], a.x, b.x),
    clampg(p[1], a.y, b.y),
    clampg(p[2], a.z, b.z),
...
vec4 smaxg(vec4 a, vec4 b, float k) {
    vec4 h = clampg(constg(0.5) + 0.5 / k * (b - a), 0.0, 1.0);
    vec4 hp = constg(1.0) - h;
    return prodg(h, b) + prodg(hp, a) + k * prodg(h, hp);
}
...
// 3d unit box hash texture
float hash3d(
  int seed,
  ivec3 p) {
  return float((next_hash(hash(p) * seed) >> 1) + (1 << 30)) / pow(2.0, 31.0); }
...
float noise3(
  ...
  vec3 w = -6.0 * (f - 1.0) * f;
  float k[8] = float[8](
    hash3d(s, ip + ivec3(0, 0, 0)),
    hash3d(s, ip + ivec3(1, 0, 0)),
    hash3d(s, ip + ivec3(0, 1, 0)),
    hash3d(s, ip + ivec3(1, 1, 0)),
    hash3d(s, ip + ivec3(0, 0, 1)),
    hash3d(s, ip + ivec3(1, 0, 1)),
    hash3d(s, ip + ivec3(0, 1, 1)),
    hash3d(s, ip + ivec3(1, 1, 1)));
  return 
     v.x * v.y * v.z * k[0]
   + u.x * v.y * v.z * k[1]
   + v.x * u.y * v.z * k[2]
...
float fractal3(
  ...
  for (int i = 0; i < harmonics; i++) {
    float q = pow(noise3(s++, freq * p), 2.0);
    v += q * ampl;
    freq *= division;
    ampl *= attenuation * pow(q, 0.2); }
  return v; }
...
vec4 fractal3g(
  ...
  int harmonics) {
  vec4 v = constg(0.0);
  vec4 ampl = constg(1.0);
  float freq = 1.0;
  for (int i = 0; i < harmonics; i++) {
    vec4 q = noise3g(s++, freq * p);
    q = powg(q, 2.0);
    v += prodg(q, ampl);
    freq *= division;
    ampl = prodg(attenuation * powg(q, 0.2), ampl); }
  return v; }
...
vec3 cosine_gen(vec3 n) {
  return axial(n, 2.0 / pi * acos(1.0 - unitrand())); }
