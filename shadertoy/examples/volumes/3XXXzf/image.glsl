...
float furDensity(vec3 pos, out vec2 uv)
{
	...
	vec4 tex = textureLod(iChannel0, uv, 0.0);
...
vec3 furShade(vec3 pos, vec2 uv, vec3 ro, float density)
{
    ...
    float st = 1.0-(dot(T,L)*dot(T,L));
    float vt = 1.0-(dot(T,V)*dot(T,V));
	//float diff = max(0.0, dot(N, L));
	//float diff = max(0.0, dot(N, L)*0.5+0.5);
    float diff = max(0.0,st/sqrt(st));
	//float spec = pow(max(0.0, dot(N, H)), shininess);
    float spec = pow(max(0.0,sqrt(1.0-dot(T,H)*dot(T,H))),shininess);
