...
const float id_inner = 1.0;
const float id_body = 2.0;

vec2 Scene(vec3 p, mat3 localToWorld)
{
	p = p * localToWorld;
...
vec3 SceneNormal(in vec3 pos, mat3 localToWorld)
{
	...
	vec3 nor = vec3(
		Scene(pos + eps.xyy, localToWorld).x - Scene(pos - eps.xyy, localToWorld).x,
		Scene(pos + eps.yxy, localToWorld).x - Scene(pos - eps.yxy, localToWorld).x,
		Scene(pos + eps.yyx, localToWorld).x - Scene(pos - eps.yyx, localToWorld).x);
...
float SceneAO(vec3 p, vec3 n, mat3 localToWorld)
{
	...
	for (int i = 0; i < 6; ++i)
	{
		...
		float t = Scene(n * off + p, localToWorld).x;
		ao += (off - t) * s;
		s *= 0.4;
	}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    q = (fragCoord.xy + o) / iResolution.xy;
#endif
	p = -1. + 2. * q;
