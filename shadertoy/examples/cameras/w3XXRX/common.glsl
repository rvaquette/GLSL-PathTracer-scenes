...
// lens high resolution
#ifdef HONEYCOMB37
const vec3 plArray[37] = vec3[37](
vec3(0.000, 0.000, 0.0),
...
vec2 map( in vec3 pos )
{
    ...
    {
      res = opU( res, vec2( sdSphere(    pos-vec3(-2.0, 0.25, 0.0), 0.25 ), 126.89 ) );
    }
    ...
    {
    // more primitives
	res = opU( res, vec2( sdPyramid(    pos-vec3(-1.0,-0.6,-3.0), 0.65 ), 3.56 ) );
...
float checkersGradBox( in vec2 p )
{
    ...
    vec2 i = 3.0/w*(abs(fract((p-0.5*w)*0.5)-0.5)-abs(fract((p+0.5*w)*0.5)-0.5));
    // xor pattern
    return i.x*i.y;                  
}
...
vec3 render_low(in vec3 ro, in vec3 rd)
{ 
    // background
    vec3 col = vec3(0.7, 0.7, 0.9);// - max(rd.y,0.0)*0.3;
    ...
    if( m>-0.5 )
    {
        ...
        // material        
        col = 0.2 + 0.2*sin( m*2.0 + vec3(0.0,1.0,2.0) ); //return col * pow(dot(rd, nor), 0.5);
        float ks = 1.0;
...
{
	vec3 cw = normalize(ta-ro); //direction vector
	vec3 cp = vec3(sin(cr), cos(cr), 0.0); // rotation
	vec3 cu = normalize( cross(cw,cp) ); //up vector
	vec3 cv =          ( cross(cu,cw) ); //right vector
